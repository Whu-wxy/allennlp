...
@DatasetReader.register("squad_filter")
class SquadReaderFilter(DatasetReader):
    """
    ...
    filt_answer_length : ``int``,需要过滤的答案长度.
    filt_answer_bLong : ``bool``，更长的过滤还是更短的.
    """
    def __init__(self,
                 ...
                 filt_answer_length: int = 5,
                 filt_answer_longer: bool = False) -> None:
        super().__init__(lazy)
        ...
        self.filt_answer_length = filt_answer_length
        self.filt_answer_longer = filt_answer_longer
    ...
    def text_to_instance(self,  # type: ignore
        ...
        #根据答案的词数进行过滤
        if self.filt_answer_length != None:
            for answer in answer_texts:
                answer_tokens = self._tokenizer.tokenize(answer)
                if self.filt_answer_longer:
                    if len(answer_tokens) > self.filt_answer_length:
                        break
                    else:
                        return None
                else:
