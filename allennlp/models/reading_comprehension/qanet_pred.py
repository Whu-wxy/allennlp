...
#两个qa_encoder+一个预测初始+一个预测终止

@Model.register("qanet_pred")
class QaNet_pred(Model):
    """
    ...
    def forward(self,  # type: ignore
        ...
        span_start_input = self._modeling_begin_layer(modeled_passage)

        # Shape: (batch_size, passage_length)
        span_start_logits = self._span_start_predictor(span_start_input).squeeze(-1)

        # Shape: (batch_size, passage_length, modeling_dim * 2)
        span_end_input = self._modeling_end_layer(modeled_passage)
