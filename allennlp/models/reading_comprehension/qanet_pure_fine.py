...
@Model.register("qanet_pure_fine")
class QaNet_pure_fine(Model):
    """
    ...
    def __init__(self, vocab: Vocabulary,
        ...
        phrase_in_dim = phrase_layer.get_input_dim()
        phrase_out_dim = phrase_layer.get_output_dim()
        ...
        self._embedding_proj_layer = torch.nn.Linear(text_embed_dim, phrase_in_dim)
        self._highway_layer = Highway(phrase_in_dim, num_highway_layers)
    ...
    def forward(self,  # type: ignore
        ...
        if not self.training:
            ...
            passage_question_attention = coattention_vectors.bmm(encoded_question.transpose(1, 2))
        elif self.training:
            passage_question_attention = None

        coattention_vectors = self._dropout(coattention_vectors)

        # modeled_passage_list = [self._modeling_proj_layer(coattention_vectors)]
        modeled_passage_list = []
        modeled_passage_list.append(coattention_vectors)
