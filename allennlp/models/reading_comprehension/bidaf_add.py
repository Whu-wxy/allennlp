...
@Model.register("bidaf_add")
class BidirectionalAttentionFlow_add(Model):
    """
    ...
    def __init__(self, vocab: Vocabulary,
                 ...
                 modeling_layer: Seq2SeqEncoder,
                 span_end_encoder: Seq2SeqEncoder,
                 add_encoder: Seq2SeqEncoder,
                 dropout: float = 0.2,
                 mask_lstms: bool = True,
                 initializer: InitializerApplicator = InitializerApplicator(),
                 regularizer: Optional[RegularizerApplicator] = None) -> None:
        super(BidirectionalAttentionFlow_add, self).__init__(vocab, regularizer)
        ...
        add_input_dim = encoding_dim * 4 + add_encoder.get_output_dim()
        self._add_pred = TimeDistributed(torch.nn.Linear(add_input_dim, 1))
...
#################################################################
        # 将end的信息加入全文信息
        # Shape: (batch_size, modeling_dim)
        span_end_representation2 = util.weighted_sum(modeled_passage, span_end_probs)
        ...
        # Shape: (batch_size, passage_length, encoding_dim * 4 + span_end_encoding_dim)
        add_input = self._dropout(torch.cat([final_merged_passage, encoded_span_begin], dim=-1))
        add_logits = self._add_pred(add_input).squeeze(-1)
        # 预测begin
        add_probs = util.masked_softmax(add_logits, passage_mask)

        span_start_logits = util.replace_masked_values(add_logits, passage_mask, -1e7)
        span_end_logits = util.replace_masked_values(span_end_logits, passage_mask, -1e7)
        best_span = get_best_span(add_logits, span_end_logits)

        output_dict = {
                "passage_question_attention": passage_question_attention,
                "span_start_logits": span_start_logits,
                "span_start_probs": add_probs,
