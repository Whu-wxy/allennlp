<<<<<<< HEAD:allennlp/models/reading_comprehension/bidaf_fine.py
import logging
...
class BidirectionalAttentionFlow_multico(Model):
    ...
    def __init__(self, vocab: Vocabulary,
        ...
        self._projection_layer = torch.nn.Linear(encoding_dim * 5, modeling_layer.get_input_dim())
...
@Model.register("bidaf_high")
class BidirectionalAttentionFlow_high(Model):
    """
    Parameters
    ----------
    vocab : ``Vocabulary``
    ...
    def __init__(self, vocab: Vocabulary,
                 ...
                 initializer: InitializerApplicator = InitializerApplicator(),
                 regularizer: Optional[RegularizerApplicator] = None) -> None:
        super(BidirectionalAttentionFlow_high, self).__init__(vocab, regularizer)

        self._text_field_embedder = text_field_embedder
        self._embedding_proj_layer = torch.nn.Linear(text_field_embedder.get_output_dim(), phrase_layer.get_input_dim())
        self._highway_layer = TimeDistributed(Highway(phrase_layer.get_input_dim(),
                                                      num_highway_layers))
        self._phrase_layer = phrase_layer
        self._matrix_attention = LegacyMatrixAttention(similarity_function)
        self._modeling_layer = modeling_layer
    ...
    def forward(self,  # type: ignore
        ...
        best_span_str : List[str]
            ...
            question.
        """
        embedded_question = self._embedding_proj_layer(self._text_field_embedder(question))
        embedded_passage = self._embedding_proj_layer(self._text_field_embedder(passage))
    ...
    def get_best_span(span_start_logits: torch.Tensor, span_end_logits: torch.Tensor) -> torch.Tensor:
        ...
        span_end_indices = best_spans % passage_length
        return torch.stack([span_start_indices, span_end_indices], dim=-1)
>>>>>>> refs/remotes/origin/master:mymodel/bidaf_high.py
